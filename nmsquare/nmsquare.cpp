...
static S_matches thr_SquareGen(unsigned long long int t_start, unsigned long long int t_limit, unsigned long long int t_step) {
    ...
    for (unsigned long long int i = t_start; i < t_limit; i += t_step) {
        ...
        for (unsigned long long int lN = 1; lN < N_limit; lN++) {
            ...
            flash:
                t_cycles++;
                if (t_matches > t_best.matches) { t_best.e = E; t_best.n = lN; t_best.m = lM; t_best.matches = t_matches + 1; }
                t_matches = 0;
            }
        }
    }
    ...
    if (t_best.matches > g_best.matches) {
        g_best.e = E; g_best.n = t_best.n; g_best.m = t_best.m; g_best.matches = t_best.matches;
    }
    std::cout << "thread " << t_start << ": " << t_cycles << " cycles, E="<< E << 
        " t_best=" << t_best.matches << " e=" << t_best.e << " n=" << t_best.n << " m=" << t_best.m << "\n";
    mlock.unlock();
    return t_best;
}
...
top:
    ...
    for (int l = g_start; l < g_start + (g_limit * g_numthreads); l += g_numthreads)
    {
        ...
        std::cout << "CURRENT g_best=" << g_best.matches << " e=" << g_best.e << " n=" << g_best.n << " m=" << g_best.m << "\n\n";
    }
    ...
    std::cout << "FINAL g_best=" << g_best.matches << " e=" << g_best.e << " n=" << g_best.n << " m=" << g_best.m << "\n\n";
